...
<body>
  ...
  renderPage = function renderPageOverride(options = {}) {
    const title = esc(options.title || "Lay Key Free");
    const heading = esc(options.heading || "Lay Key Free");
    const sub = esc(
      options.sub || "Giao dien moi giu nguyen luong lay key, callback va API hien tai."
    );
    const alertClass =
      options.alertType === "err" ? "err" : options.alertType === "ok" ? "ok" : "";
    const alertText = options.alert ? esc(options.alert) : "";
    const freeNotice = esc(
      options.notice ||
        "Vuot link xong co key han dung 1 ngay, moi ngay chi lay duoc 1 key. Nhung key Huy share thi co han nhieu hon 1 ngay nen anh em lay khong kip thi vo day lay key 1 ngay xai nhe."
    );
    const featureGrid = `
      <div class="freeInfoGrid">
        <div class="freeInfoItem">
          <span class="freeInfoDot"></span>
          <div>
            <strong>Moi ngay 1 key</strong>
            <span>Moi thiet bi chi nhan duoc 1 key free trong 24 gio.</span>
          </div>
        </div>
        <div class="freeInfoItem">
          <span class="freeInfoDot"></span>
          <div>
            <strong>Han dung 1 ngay</strong>
            <span>Key free dung de anh em co key tam thoi neu chua kip lay key share.</span>
          </div>
        </div>
        <div class="freeInfoItem">
          <span class="freeInfoDot"></span>
          <div>
            <strong>Vuot link xong moi co key</strong>
            <span>Hoan tat callback hop le thi he thong moi tu tao key cho ban.</span>
          </div>
        </div>
      </div>
    `;
    const captchaMarkup = FREE_TURNSTILE_SITE_KEY
      ? `
        <div class="tile freeSoftCard" style="margin-top:14px">
          <div class="sub" style="margin:0 0 10px">Xac minh captcha truoc khi tao phien free.</div>
          <div class="cf-turnstile" data-sitekey="${esc(FREE_TURNSTILE_SITE_KEY)}" data-theme="dark"></div>
        </div>
      `
      : "";
    const keyBlock = options.key
      ? `
        <div class="tile freeKeyCard" style="margin-top:14px">
          <div class="freeSectionLabel">Key da nhan</div>
          <div id="freeKeyValue" class="freeKeyValue">${esc(options.key)}</div>
          <div class="sub" style="margin-top:10px">Han dung: ${esc(options.expireText || "")}</div>
        </div>
        <div class="grid2 freeActionGrid" style="margin-top:12px">
          <button id="copyKeyBtn" class="btn freePrimaryBtn" style="margin-top:0">Copy key</button>
          <a class="smallBtn freeGhostBtn" href="/panel" style="height:56px">Mo panel</a>
        </div>
      `
      : `
        <div class="freeHeroPanel" style="margin-top:14px">
          <div class="freeSectionLabel">Luu y quan trong</div>
          <div class="freeNoticeText">${freeNotice}</div>
          ${featureGrid}
        </div>
        ${captchaMarkup}
        <button id="freeStartBtn" class="btn freePrimaryBtn" style="margin-top:14px">Lay Key Free</button>
      `;
              ...
              try {
                ...
                if (!current) {
                  if (window.crypto && window.crypto.randomUUID) {
                    ...
                    current = "ath-" + Math.random().toString(36).slice(2) + "-" + Date.now().toString(36);
                  }
                  localStorage.setItem(key, current);
                }
              } catch (err) {
            ...
            button.addEventListener("click", async function () {
              ...
              try {
                ...
                if (!data.ok) {
                  ...
                  button.disabled = false;
                  button.textContent = "Lay Key Free";
                  return;
                }
                status.className = "msg ok";
...
<head>
  ...
  ${FREE_TURNSTILE_SITE_KEY ? '<script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script>' : ""}
  <style>
    .freeCard{max-width:720px}
    .freeHeroPanel{
      padding:18px;
      border-radius:24px;
      border:1px solid rgba(182,147,255,.22);
      background:
        radial-gradient(circle at top right, rgba(176,129,255,.14), transparent 42%),
        linear-gradient(180deg, rgba(18,20,28,.96), rgba(10,11,16,.98));
      box-shadow:0 18px 48px rgba(5,6,10,.34);
    }
    .freeSectionLabel{
      font-size:12px;
      line-height:1;
    ...
    .freeInfoDot{
      ...
      border-radius:999px;
      margin-top:5px;
      background:linear-gradient(135deg,#8c68ff,#f4df9f);
      box-shadow:0 0 18px rgba(155,112,255,.45);
      flex:0 0 auto;
    }
    .freeSoftCard{
      border-color:rgba(255,255,255,.08);
      background:linear-gradient(180deg, rgba(22,24,33,.92), rgba(12,13,19,.95));
    }
    .freeKeyCard{
      border-color:rgba(244,223,159,.2);
      background:
        radial-gradient(circle at center, rgba(182,147,255,.08), transparent 56%),
        linear-gradient(180deg, rgba(18,20,28,.96), rgba(10,11,16,.98));
    }
    .freeKeyValue{
      font-size:30px;
      ...
      color:#efe8ff;
      text-shadow:0 0 26px rgba(176,129,255,.16);
    }
    .freePrimaryBtn,
    .freeGhostBtn{
    ...
    .freePrimaryBtn:hover,
    .freePrimaryBtn:focus-visible{
      transform:translateY(-2px);
      box-shadow:0 16px 34px rgba(18,20,28,.35), 0 0 0 1px rgba(244,223,159,.12) inset;
    }
    .freeGhostBtn:hover,
    .freeGhostBtn:focus-visible{
      transform:translateY(-2px);
      border-color:rgba(182,147,255,.38);
      box-shadow:0 14px 28px rgba(8,9,12,.28);
    }
    @media (max-width:700px){
      .freeInfoGrid{grid-template-columns:1fr}
    }
  </style>
</head>
<body>
  ...
  <div class="wrap">
    <div class="card freeCard">
      ...
      <div class="content">
        <div class="noticeBox" style="line-height:1.7">
          Vuot link xong co key han dung 1 ngay, moi ngay chi lay duoc 1 key, nhung key Huy share thi co han nhieu hon 1 ngay nen anh em lay khong kip thi vo day lay key 1 ngay xai nhe.
        </div>
        ${keyBlock}
        <div id="freeStatus" class="msg ${alertClass}" style="margin-top:14px">${alertText}</div>
  ...
  app.use((req, res, next) => {
    ...
    if (
      typeof deps.shouldShowAndroidUpdate === "function" &&
      deps.shouldShowAndroidUpdate(req)
    ) {
      const message =
        deps.androidUpdateMessage ||
      ...
      if (req.method === "GET" && typeof deps.renderAndroidUpdateHtml === "function") {
        return res.send(deps.renderAndroidUpdateHtml());
      }
      return res.status(403).json({ ok: false, msg: message });
    }
    if (!freeReady) {
