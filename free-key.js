...
<body>
  ...
  renderPage = function renderPageOverride(options = {}) {
    const title = esc(options.title || "Lấy Key Free");
    const heading = esc(options.heading || "Lấy Key Free");
    const sub = esc(
      options.sub || "Kết Nối Zalo : 0818 249 250 Nếu Anh Em Muốn Múc Key Vĩnh Viễn !"
    );
    const alertClass =
      options.alertType === "err" ? "err" : options.alertType === "ok" ? "ok" : "";
    const alertText = options.alert ? esc(options.alert) : "";
    const freeNotice = esc(
      options.notice ||
        "Vượt link xong có key hạn dùng 1 ngày, mỗi ngày chỉ lấy được 1 key. Những key Huy share thì có hạn nhiều hơn 1 ngày nên anh em lấy không kịp thì vô đây lấy key 1 ngày xài nhé."
    );
    const featureGrid = `
      <div class="freeInfoGrid">
        <div class="freeInfoItem">
          ...
          <div>
            <strong>Mỗi ngày 1 key</strong>
            <span>Mỗi thiết bị chỉ nhận được 1 key free trong 24 giờ.</span>
          </div>
        </div>
        <div class="freeInfoItem">
          <span class="freeInfoDot"></span>
          <div>
            <strong>Hạn dùng 1 ngày</strong>
            <span>Key free dùng để anh em có key tạm thời nếu chưa kịp lấy key share.</span>
          </div>
        </div>
        <div class="freeInfoItem">
          <span class="freeInfoDot"></span>
          <div>
            <strong>Vượt link xong mới có key</strong>
            <span>Hoàn tất callback hợp lệ thì hệ thống mới tự tạo key cho bạn.</span>
          </div>
        </div>
      </div>
    `;
    ...
    const keyBlock = options.key
      ? `
        <div class="tile freeKeyCard" style="margin-top:14px">
          <div class="freeSectionLabel">Key đã nhận</div>
          <div id="freeKeyValue" class="freeKeyValue">${esc(options.key)}</div>
          <div class="sub" style="margin-top:10px">Hạn dùng: ${esc(options.expireText || "")}</div>
        </div>
        <div class="grid2 freeActionGrid" style="margin-top:12px">
          ...
          <a class="smallBtn freeGhostBtn" href="/panel" style="height:56px">Mở panel</a>
        </div>
      `
      : `
        <div class="freeHeroPanel" style="margin-top:14px">
          <div class="freeSectionLabel">Lưu ý quan trọng</div>
          <div class="freeNoticeText">${freeNotice}</div>
          ${featureGrid}
        </div>
        ${captchaMarkup}
        <button id="freeStartBtn" class="btn freePrimaryBtn" style="margin-top:14px">Lấy Key Free</button>
      `;
            ...
            button.addEventListener("click", async function () {
              ...
              try {
                ...
                if (!data.ok) {
                  ...
                  status.textContent = data.msg || "Không tạo được phiên";
                  button.disabled = false;
                  button.textContent = "Lấy Key Free";
                  return;
                }
                status.className = "msg ok";
                status.textContent = "Đang mở Link4m...";
