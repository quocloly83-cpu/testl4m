...
module.exports = function registerFreeKeyRoutes(app, deps) {
  ...
  function cleanupFreeStore() {
    ...
    Object.keys(freeStore.claims || {}).forEach((claimId) => {
      ...
      if (claim.status === "claimed" && claimedAt > 0 && claimedAt + FREE_CLAIM_HISTORY_MS > now) {
        keepClaims[claimId] = claim;
        return;
      }
      if (expiresAt > now || expiresAt + FREE_CLAIM_HISTORY_MS > now) {
        keepClaims[claimId] = claim;
      }
    });
  ...
  function cleanupExpiredFreeKeys() {
    const now = Date.now();
    let changed = false;
    const next = {};
  ...
  function saveFreeKeys(options = {}) {
    ...
    if (options.background) return Promise.resolve();
