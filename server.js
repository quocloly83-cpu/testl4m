...
<head>
  ...
  <script>
        ...
        try {
          ...
          xhr.onreadystatechange = function () {
            ...
            try {
              ...
              reject(new Error("server_khong_tra_json"));
            }
          };
    ...
    async function dangNhap(event) {
      ...
      setMsg("Đang kiểm tra key...", "ok");
      try {
        var device = getDevice();
        var result;
        if (isIOS()) {
          var qs = "?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device);
          result = await requestJson("GET", "/api/check-ios" + qs, null, 15000);
        } else {
          result = await requestJson("POST", "/api/check", { key: key, device: device }, 15000);
        }
        var data = result.data || {};
        if (!data.ok) {
          setMsg(data.msg || data.error || ("Đăng nhập thất bại. HTTP " + result.status), "err");
          return false;
        }
        ...
        setTimeout(function () { moPanel(data); }, 260);
        return false;
      } catch (e) {
        setMsg("Lỗi kết nối server: " + (e && e.message ? e.message : "không rõ"), "err");
        return false;
      } finally {
        if (loginButtonEl) {
    ...
    if (loginButtonEl) {
      loginButtonEl.onclick = dangNhap;
      loginButtonEl.addEventListener("touchend", dangNhap, { passive: false });
      loginButtonEl.addEventListener("pointerup", dangNhap);
    }
    if (keyInputEl) {
