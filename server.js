...
<head>
  ...
  <script>
    ...
    function postCheckKey(payload, timeoutMs) {
      return new Promise(function (resolve, reject) {
        ...
        xhr.onreadystatechange = function () {
          ...
          try { data = JSON.parse(xhr.responseText || "{}"); } catch (e) {
            return reject(new Error("bad_json:" + String(xhr.responseText || "").slice(0, 120)));
          }
          data.__status = xhr.status;
          resolve(data);
        };
    ...
    async function dangNhap() {
      ...
      try {
        const data = await postCheckKey({ key: key, device: getDevice() }, 12000);
        if (!data || !data.ok) {
...
app.get("/api/check", (req, res) => {
  ...
  req.body = {
    key,
    device: String((req.query && req.query.device) || "ios-test-device").trim()
  };
  return handleCheckKey(req, res);
});
...
async function handleCheckKey(req, res) {
  try {
    ...
    const key = String(body.key || (req.query && req.query.key) || "").trim();
    const device = String(body.device || (req.query && req.query.device) || "").trim();
    ...
    recordDailyDeviceUsage(record.scope === "free" ? "free" : "admin", platform === "android" ? "android" : "ios", device);
    ...
    return res.status(500).json({ ok: false, msg: "Server lỗi khi kiểm tra key" });
  }
}

app.post("/api/check", handleCheckKey);
