...
const ANDROID_UPDATE_MESSAGE =
  "Android đã update lên bản mới fix lỗi tâm ảo to và thêm một số tâm ảo mới. Tải ở link bio kênh TikTok : Huy Fanta hoặc YouTube : Huy Fanta nè, xem là lấy link mới nhất.";
const ANDROID_OLD_NATIVE_APK_MESSAGE =
  "Android đã update lên bản mới fix lỗi tâm ảo to và thêm một số tâm ảo mới. Tải ở link bio kênh TikTok : Huy Fanta hoặc YouTube : Huy Fanta nè, xem là lấy link mới nhất. Bản mới có thể cài đè lên bản này hoặc xóa bản này tải bản mới ở bio cũng được.";
const MIN_NATIVE_ANDROID_VERSION_CODE = Math.max(
  1,
  Number(process.env.MIN_NATIVE_ANDROID_VERSION_CODE || 7) || 7
);
const PANEL_IOS_SNOW_SPECS = [
  ["4%", 3, 26, -2, 0.18, 10],
  ["9%", 4, 29, -8, 0.22, 14],
  ["14%", 5, 31, -13, 0.28, 18],
  ["20%", 3, 28, -6, 0.17, 11],
...
function getNativeApkVersionCode(req) {
  const raw = String(req.headers["x-aimtrickhead-version"] || "").trim();
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed > 0 ? Math.trunc(parsed) : 0;
}

function shouldBlockOldNativeApk(req) {
  if (!isNativeApkClient(req)) return false;
  return getNativeApkVersionCode(req) < MIN_NATIVE_ANDROID_VERSION_CODE;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(
      safeUrl,
      {
        ...
        timeout: 12000,
        headers: {
          "User-Agent": "Mozilla/5.0 (Linux; Android 13; AimTrickHead) AppleWebKit/537.36 Chrome/124.0.0.0 Mobile Safari/537.36",
...
async function resolveMediaFireDirectUrl(url) {
  const safeUrl = sanitizeExternalUrl(url);
  if (!/mediafire\.com/i.test(safeUrl)) return safeUrl;
...
function renderSnowFlakes(flakeClass, specs) {
  return specs
    .map(
      ([left, size, duration, delay, opacity, drift]) =>
        `<span class="${flakeClass}" style="left:${left};width:${size}px;height:${size}px;animation-duration:${duration}s;animation-delay:${delay}s;--snowOpacity:${opacity};--snowDrift:${drift}px"></span>`
    )
    .join("\n");
}
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .wrap{
      ...
      .cardSnowFlake{
        ...
        background:radial-gradient(circle at 35% 35%, rgba(255,255,255,.98), rgba(255,255,255,.30) 70%, transparent 72%);
    ...
    .loginIconRow{display:flex;align-items:center;justify-content:center;gap:clamp(48px,10vw,76px);margin-top:22px}
    .loginIconLink{
    ...
    .loadingLayer.hide{opacity:0;visibility:hidden;pointer-events:none}
    .loadingLogo{
    ...
    html.ios-performance-mode body:before,
    html.ios-performance-mode body:after,
    html.ios-performance-mode .bgAura,
    html.ios-performance-mode .cardSnowLayer,
    html.ios-performance-mode .top::after,
    html.ios-performance-mode .liveFx::after,
    html.ios-performance-mode .tile::before{
      display:none !important;
    }
    html.ios-performance-mode .card,
    html.ios-performance-mode .top,
    html.ios-performance-mode .statChip,
    html.ios-performance-mode .btn,
    html.ios-performance-mode .pill,
    html.ios-performance-mode .tab,
    html.ios-performance-mode .homeGlass,
    html.ios-performance-mode .fxOverlay{
      backdrop-filter:none !important;
      -webkit-backdrop-filter:none !important;
    }
    html.ios-performance-mode .iosSnowFlake,
    html.ios-performance-mode .cardSnowFlake,
    html.ios-performance-mode .orb,
    html.ios-performance-mode .fxGlow,
    html.ios-performance-mode .fxOverlayCard::before,
    html.ios-performance-mode .fxOverlayCard::after,
    html.ios-performance-mode .gameConfirmCard::before,
    html.ios-performance-mode .gameConfirmCard::after{
      filter:none !important;
    }
    html.ios-performance-mode .card,
    html.ios-performance-mode .top .logoBox,
    html.ios-performance-mode .title,
    html.ios-performance-mode .loadingLogo,
    html.ios-performance-mode .loadingText,
    html.ios-performance-mode .loadingBar > span,
    html.ios-performance-mode .fxMeter:before,
    html.ios-performance-mode .prepSpinner,
    html.ios-performance-mode .prepBar span,
    html.ios-performance-mode .fxCursor{
      animation:none !important;
    }
    html.ios-performance-mode .card{
      box-shadow:0 22px 64px rgba(0,0,0,.42), inset 0 1px 0 rgba(255,255,255,.03), inset 0 -1px 0 rgba(255,255,255,.02);
    }
    html.ios-performance-mode .liveFx,
    html.ios-performance-mode .fxOverlayCard,
    html.ios-performance-mode .gameConfirmCard{
      box-shadow:0 18px 40px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.03);
    }
    html.ios-performance-mode .loadingLogo{
      box-shadow:0 16px 36px rgba(0,0,0,.22);
    }
    html.ios-performance-mode .liveFx.activePulse{
      box-shadow:0 18px 40px rgba(4,0,18,.24), inset 0 0 0 1px rgba(255,255,255,.03);
    }
    html.ios-performance-mode .monitorGameBtn,
    html.ios-performance-mode .socialBtn,
    html.ios-performance-mode .gameBtn,
    html.ios-performance-mode .smallBtn,
    html.ios-performance-mode .tab,
    html.ios-performance-mode .tile,
    html.ios-performance-mode .btn{
    ...
    #panelView.panelReveal #liveFxBox{animation-delay:.2s}
      ...
      .credit{font-size:11px}
        ...
        .loginIconRow{gap:32px;margin-top:20px}
  ...
  <div class="wrap">
    <div class="androidUpdateWrap">
      <div class="androidUpdateCard">
        ...
        <div class="androidUpdateLead">Android đã update lên bản mới.</div>
          <div class="androidUpdateBody">
            Android đã update lên bản mới <b>fix lỗi tâm ảo to</b> và thêm một số <b>tâm ảo mới</b>.<br><br>
            Tải ở <b>link bio kênh TikTok : Huy Fanta</b> hoặc <b>YouTube : Huy Fanta nè</b>, xem là lấy <b>link mới nhất</b>.
          </div>
          <div class="androidUpdateActions">
            ...
            <a class="socialBtn" href="${YOUTUBE_URL}" target="_blank" rel="noopener noreferrer">${iconYouTube(20)} <span>YouTube Huy Fanta</span></a>
          </div>
...
function renderNativeApkDownloadHtml(downloadUrl) {
  const safeUrl = sanitizeExternalUrl(downloadUrl);
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover">
  <title>Tải Bản Cập Nhật AimTrickHead</title>
  <style>
    ...
    *{box-sizing:border-box}
    body{
      ...
      background:radial-gradient(circle at top, rgba(164,122,255,.18), transparent 32%),linear-gradient(180deg,#08090d,#11131a 54%,#090a0f);
      color:#fff
    }
    .card{
      width:min(92vw,560px);border-radius:28px;padding:28px 22px;
      background:linear-gradient(180deg, rgba(18,20,28,.96), rgba(10,11,17,.98));
      border:1px solid rgba(255,255,255,.08);
      box-shadow:0 24px 80px rgba(0,0,0,.44), inset 0 1px 0 rgba(255,255,255,.05)
    }
    .badge{
      display:inline-flex;align-items:center;justify-content:center;padding:9px 14px;border-radius:999px;
      border:1px solid rgba(255,255,255,.09);background:rgba(255,255,255,.04);font-size:12px;font-weight:700;letter-spacing:.08em
    }
    h1{margin:18px 0 10px;font-size:34px;line-height:1.04}
    p{margin:0;color:#dad7e6;line-height:1.7}
    .hint{margin-top:12px;color:#e6d4a6;font-weight:700}
    .cta{
      display:flex;align-items:center;justify-content:center;text-align:center;text-decoration:none;
      min-height:62px;width:100%;margin-top:24px;padding:0 18px;border-radius:20px;
      background:linear-gradient(135deg,#9f75ff,#f0deb1 58%,#f6f2e8);
      color:#111;font-weight:900;font-size:18px;border:0;box-shadow:0 16px 40px rgba(168,132,255,.24)
    }
    .meta{margin-top:14px;font-size:13px;color:#bdb7cf}
  </style>
</head>
<body>
  <div class="card">
    <div class="badge">BẢN APK MỚI</div>
    <h1>Cập nhật AimTrickHead</h1>
    <p>App đã cập nhật thêm tính năng lấy độ nhạy và fix một số lỗi. Không xóa bản cũ. Hãy bấm nút bên dưới để tải APK mới và cài đè như bình thường.</p>
    <p class="hint">Nếu trang không tự chuyển, bấm trực tiếp vào nút tải APK bên dưới.</p>
    <a class="cta" href="${escapedUrl}" id="downloadBtn">ẤN NÚT NÀY ĐỂ TẢI BẢN CẬP NHẬT</a>
    <div class="meta">Link tải: ${escapedUrl || "Chưa cấu hình"}</div>
  </div>
  <script>
    (function () {
...
<head>
  ...
  <meta name="apple-mobile-web-app-capable" content="yes">
    ...
    <style>
      ...
      .iosSnowFlake{
        position:absolute;top:-12vh;border-radius:50%;
        background:radial-gradient(circle at 35% 35%, rgba(255,255,255,.96), rgba(255,255,255,.24) 70%, transparent 72%);
    ...
    <script>
      (function () {
        ...
        var isIOS = /iPhone|iPad|iPod/i.test(ua);
        if (isIOS) {
          document.documentElement.classList.add("ios-performance-mode");
        }
        document.addEventListener("DOMContentLoaded", function () {
  ...
  <body>
    ...
    <div class="iosSnowLayer" id="iosSnowLayer" aria-hidden="true">
      ${renderSnowFlakes("iosSnowFlake", PANEL_IOS_SNOW_SPECS)}
        </div>
    <div class="statusBar">
  ...
  <div class="loadingLayer" id="loadingLayer">
    ...
    <div class="wrap">
      <div class="card">
        <div class="cardSnowLayer" id="cardSnowLayer" aria-hidden="true"></div>
        <div class="top">
          <div class="brand">
        ...
        <div id="loginView" class="loginHero">
          <div class="loginFrame">
            ...
            <div class="loginFormStack">
              ...
              <button id="loginButton" class="btn loginPrimaryBtn" type="button" onclick="return window.__athLoginTap ? window.__athLoginTap(event) : (dangNhap(), false)" ontouchend="return window.__athLoginTap ? window.__athLoginTap(event) : (dangNhap(), false)">Đăng nhập</button>
            </div>
            ...
            <a class="socialBtn loginFreeBtn" href="/free">Lấy Key Free !</a>
            ...
            <script>
              (function () {
                ...
                function setBusy(isBusy) {
                  ...
                  loginButton.textContent = isBusy ? "Đang kiểm tra..." : "Đăng nhập";
                }
                ...
                function getDevice() {
                  var id = safeStorageGet("ath_device");
                  if (!id) {
                    id = "web-" + Math.random().toString(36).slice(2, 12);
                    safeStorageSet("ath_device", id);
                  }
                  return id;
                }
                window.__athLoginTap = function (event) {
                  if (event) {
                  ...
                  try {
                    ...
                    xhr.onreadystatechange = function () {
                      ...
                      try {
                        ...
                        safeStorageSet("ath_session", data.token || "");
                        safeStorageSet("ath_key", key);
                    ...
                    xhr.ontimeout = function () {
                      setBusy(false);
                      setMsg("Máy chủ phản hồi quá chậm.", "err");
                    };
                    xhr.send(JSON.stringify({ key: key, device: getDevice() }));
                  } catch (e) {
                    setBusy(false);
                    setMsg("Không thể kết nối tới máy chủ.", "err");
                  }
        ...
        <div id="panelView" class="hidden">
          ...
          <div id="tab3" class="tabPane">
            <div class="tile">
              ...
              <p class="desc">Ô tên máy sẽ tự điền theo thiết bị hiện tại nếu trình duyệt lấy được. Kết quả giữ ổn định theo tên máy và chỉ đổi khi bấm reset.</p>
  ...
  <script>
    ...
    function getDevice() {
      let id = safeStorageGet("ath_device");
      if (!id) {
        id = "web-" + Math.random().toString(36).slice(2, 12);
        safeStorageSet("ath_device", id);
      }
    ...
    function setLoginBusy(isBusy) {
      ...
      loginButtonEl.disabled = !!isBusy;
      loginButtonEl.dataset.busy = isBusy ? "1" : "0";
      loginButtonEl.textContent = isBusy ? "Đang kiểm tra..." : "Đăng nhập";
    }
    ...
    function buildApiUrl(apiPath) {
      ...
      if (!origin) {
        origin = String(window.location.protocol || "https:") + "//" + String(window.location.host || "");
      }
      if (path.charAt(0) !== "/") {
        path = "/" + path;
      }
    ...
    function apiRequest(apiPath, payload, timeoutMs) {
      const url = buildApiUrl(apiPath);
      const timeout = Math.max(4000, Number(timeoutMs || 12000));
      const preferXhr =
      ...
      if (preferXhr || typeof fetch !== "function") {
        return xhrRequest();
      }
      ...
      return new Promise(function (resolve, reject) {
        ...
        const timer = setTimeout(function () {
          ...
          xhrRequest().then(resolve).catch(reject);
        }, timeout);
    ...
    function saveSession(data) {
      safeStorageSet("ath_session", data.token || "");
      safeStorageSet("ath_key", data.key || "");
    }

    function getSession() { return safeStorageGet("ath_session"); }
    function getSavedKey() { return safeStorageGet("ath_key") || ""; }
    ...
    function getDevicePresetDraftKey() {
      return "ath_sensi_device_profile_name";
    }
    function getDevicePresetResetKey(normalizedName) {
      return "ath_sensi_device_profile_reset_" + normalizedName;
    }
    function clearSession() {
      safeStorageRemove("ath_session");
      safeStorageRemove("ath_key");
    }
    ...
    function simulateCodeRun(label, isOn) {
      ...
      const moduleName = String(label || "").replace(/\s+/g, "_").toLowerCase();
      const seq = [
    ...
    async function dangNhap() {
      ...
      if (!key) {
        setMsg("Vui lòng nhập key.", "err");
        showToast("Vui lòng nhập key", "err");
        return;
      }
      setLoginBusy(true);
      setMsg("Đang kiểm tra key...");
      showToast("Đang kiểm tra key...", "ok", 1600);
      try {
        const data = await apiRequest("/api/check", { key: key, device: getDevice() }, 14000);
        if (!data.ok) {
          setMsg(data.msg || "Đăng nhập thất bại.", "err");
          return;
        }
    ...
    function updateSensi(val) {
      ...
      safeStorageSet(getSensiStorageKey(), String(val));
    ...
    function seededRange(seedBase, salt, min, max) {
      ...
      return min + (stableHash(seedBase + "|" + salt) % span);
    }

    function parseIphoneNumber(name) {
      const match = name.match(/iphone\s*(\d{1,2})/);
      if (match && match[1]) return Number(match[1]) || 0;
      if (/iphone\s*xr|iphone\s*xs|iphone\s*x\b/.test(name)) return 10;
      if (/iphone\s*se/.test(name)) return 11;
      return 0;
    }
    ...
    function detectCurrentDeviceName() {
      ...
      const screenWidth = Math.round(Math.min(window.screen.width, window.screen.height) * dpr);
      const screenHeight = Math.round(Math.max(window.screen.width, window.screen.height) * dpr);
      const sizeKey = screenWidth + "x" + screenHeight;

      if (/iphone|ipad|ipod/.test(normalizedUa)) {
        const iosMap = {
          "640x1136": "iPhone 5 / SE",
          "750x1334": "iPhone 6 / 7 / 8",
          "1080x1920": "iPhone 8 Plus",
          ...
          "1290x2796": "iPhone 14 Pro Max / 15 Pro Max / 16 Pro Max",
          "1320x2868": "iPhone 16 Pro Max / 17 Pro Max"
        };
        return iosMap[sizeKey] || "iPhone";
      }

      const androidMatch = ua.match(/Android[^;)]*;\s*([^;)]+?)(?:\s+Build|\))/i);
      if (androidMatch && androidMatch[1]) {
        const rawModel = String(androidMatch[1]).replace(/;\s*wv/i, "").trim();
        if (rawModel && !/^linux$/i.test(rawModel)) return rawModel;
      }

      if (/android/i.test(normalizedUa)) return "Android";
      return "";
    }

    function classifyDevicePreset(name) {
      const normalized = normalizePresetDeviceName(name);
      const iphoneNumber = parseIphoneNumber(normalized);
      const hasIphone = /iphone|ipad|ios/.test(normalized);
      const hasProMax = /pro\s*max/.test(normalized);
      const hasPlus = /\bplus\b/.test(normalized);
      if (hasIphone) {
        if (hasProMax && iphoneNumber >= 13 && iphoneNumber <= 17) {
          return { key: "iphone_pro_max", label: "iPhone 13 Pro Max - 17 Pro Max", lookMin: 145, lookMax: 165 };
        }
        if (iphoneNumber >= 4 && iphoneNumber <= 8 && !hasPlus) {
          return { key: "iphone_legacy", label: "iPhone 4 - 8", lookMin: 185, lookMax: 190 };
        }
        return { key: "iphone_standard", label: "iPhone 8 Plus - 13", lookMin: 160, lookMax: 175 };
      }

      if (/(android yeu|redmi 9a|redmi 10a|redmi 12c|redmi 13c|oppo a|realme c|vivo y|galaxy a04|galaxy a05|galaxy a06|galaxy a10|galaxy a12|galaxy a13|galaxy a14|galaxy a15|tecno spark|infinix smart|moto e|narzo 50a)/.test(normalized) || /\ba0[4-9]\b|\ba1[0-6]\b|\ba2[0-4]\b/.test(normalized)) {
        return { key: "android_weak", label: "Android yếu", lookMin: 185, lookMax: 200 };
      }

      if (/(android manh|ultra|rog|redmagic|legion|pixel 7|pixel 8|pixel 9|oneplus 11|oneplus 12|oneplus 13|find x|iqoo|realme gt|mi 13|mi 14|xiaomi 13|xiaomi 14|14t pro|13t pro|vivo x)/.test(normalized) || /\bs2[1-5]\b|\bnote 2[0-5]\b/.test(normalized)) {
        return { key: "android_strong", label: "Android mạnh", lookMin: 170, lookMax: 185 };
      }

      return { key: "android_medium", label: "Android trung bình", lookMin: 176, lookMax: 188 };
    }

    function buildDevicePresetProfile(deviceName, resetCount) {
      const normalized = normalizePresetDeviceName(deviceName);
      if (!normalized) return null;
      const tier = classifyDevicePreset(normalized);
      const seedBase = normalized + "|" + resetCount;
      return {
        deviceName: String(deviceName || "").trim(),
        normalizedName: normalized,
        tier: tier,
        resetCount: resetCount,
        lookAround: seededRange(seedBase, "look", tier.lookMin, tier.lookMax),
        redDot: seededRange(seedBase, "red", 150, 160),
        scope2x: seededRange(seedBase, "2x", 130, 150),
        scope4x: seededRange(seedBase, "4x", 150, 170)
      };
    }

    function setDevicePresetLoading(loading) {
      if (devicePresetLoading) devicePresetLoading.classList.toggle("hidden", !loading);
      if (devicePresetGetBtn) devicePresetGetBtn.disabled = !!loading;
      if (devicePresetResetBtn) devicePresetResetBtn.disabled = !!loading;
    }

    function renderDevicePresetProfile(profile) {
      if (!devicePresetResult) return;
      if (!profile) {
        devicePresetResult.classList.add("hidden");
        return;
      }
      devicePresetResult.classList.remove("hidden");
      if (devicePresetTier) devicePresetTier.textContent = "HỒ SƠ: " + String(profile.deviceName || "").toUpperCase();
      if (devicePresetName) devicePresetName.textContent = "Thiết bị hiện chọn: " + profile.deviceName;
      if (devicePresetLook) devicePresetLook.textContent = String(profile.lookAround);
      if (devicePresetRedDot) devicePresetRedDot.textContent = String(profile.redDot);
      if (devicePreset2x) devicePreset2x.textContent = String(profile.scope2x);
      if (devicePreset4x) devicePreset4x.textContent = String(profile.scope4x);
    }

    function restoreDevicePresetProfile() {
      if (!devicePresetInput) return;
      const savedName = String(safeStorageGet(getDevicePresetDraftKey()) || "").trim();
      const autoDetectedName = detectCurrentDeviceName();
      const resolvedName = savedName || autoDetectedName;
      if (!resolvedName.trim()) {
        renderDevicePresetProfile(null);
        return;
      }
      devicePresetInput.value = resolvedName;
      if (!savedName && autoDetectedName) {
        safeStorageSet(getDevicePresetDraftKey(), autoDetectedName);
      }
      const normalized = normalizePresetDeviceName(resolvedName);
      const resetCount = Number(safeStorageGet(getDevicePresetResetKey(normalized)) || "0") || 0;
      renderDevicePresetProfile(buildDevicePresetProfile(resolvedName, resetCount));
    }

    function requestDevicePresetProfile(resetSeed) {
      if (!devicePresetInput) return;
      const deviceName = String(devicePresetInput.value || "").trim();
      if (!deviceName) {
        showToast("Vui lòng nhập tên thiết bị trước.", "err");
        return;
      }
      safeStorageSet(getDevicePresetDraftKey(), deviceName);
      const normalized = normalizePresetDeviceName(deviceName);
      let resetCount = Number(safeStorageGet(getDevicePresetResetKey(normalized)) || "0") || 0;
      if (resetSeed) {
        resetCount += 1;
        safeStorageSet(getDevicePresetResetKey(normalized), String(resetCount));
      }
      setDevicePresetLoading(true);
      renderDevicePresetProfile(null);
      window.clearTimeout(window.__devicePresetTimer);
      window.__devicePresetTimer = window.setTimeout(function () {
        const profile = buildDevicePresetProfile(deviceName, resetCount);
        setDevicePresetLoading(false);
        renderDevicePresetProfile(profile);
        if (profile) {
          simulateCodeRun("Device_Sensi_" + profile.lookAround, true);
          showToast("Đã setup độ nhạy cho " + profile.deviceName, "ok", 2600);
        }
      }, 1350);
    }
    ...
    function luuTrangThai() {
      ...
      safeStorageSet(getStateStorageKey(), JSON.stringify(state));
    }

    function taiTrangThai(key) {
      try {
        const state = Object.assign(getDefaultState(), JSON.parse(safeStorageGet(getStateStorageKey(key)) || "{}"));
        ["f1","f2","f3","f4","f5","f6","f7"].forEach(function (id) {
          const el = document.getElementById(id);
          if (el) el.checked = !!state[id];
        });
        const savedSensi = safeStorageGet(getSensiStorageKey(key)) || "60";
    ...
    if (devicePresetInput) {
      devicePresetInput.addEventListener("input", function () {
        safeStorageSet(getDevicePresetDraftKey(), devicePresetInput.value || "");
      });
    }
    bindFastTap(loginButtonEl, function () {
      dangNhap();
    });
    if (loginButtonEl) {
      window.__athLoginTap = function (event) {
      ...
      loginButtonEl.addEventListener("touchstart", function (event) {
        event.preventDefault();
      }, { passive: false });
      loginButtonEl.addEventListener("pointerup", function (event) {
        event.preventDefault();
        dangNhap();
      });
    }
    if (keyInputEl) {
      keyInputEl.addEventListener("keydown", function (event) {
        if (event.key === "Enter") {
          event.preventDefault();
          dangNhap();
        }
    ...
    async function bootHomePage() {
      ...
      if (!token) return;
      try {
        const data = await apiRequest("/api/status", { token: token, device: getDevice() }, 12000);
        if (data.ok) {
...
app.get("/android-apk-update", async (req, res) => {
  setNoCacheHeaders(res);
  const requestedUrl = sanitizeExternalUrl(req.query.url || "");
  if (!requestedUrl) {
    return res.send(renderNativeApkDownloadHtml(""));
  }
  try {
    const directUrl = await resolveMediaFireDirectUrl(requestedUrl);
    if (directUrl && directUrl !== requestedUrl) {
      return res.redirect(302, directUrl);
    }
  } catch (e) {}
  res.send(renderNativeApkDownloadHtml(requestedUrl));
});
...
app.post("/api/check", async (req, res) => {
  ...
  if (shouldBlockOldNativeApk(req)) {
    return res.json({
      ok: false,
      msg: ANDROID_OLD_NATIVE_APK_MESSAGE
    });
  }
  syncExpiredMainKeysInBackground();
  const key = String(req.body.key || "").trim();
...
app.post("/api/status", async (req, res) => {
  ...
  if (shouldBlockOldNativeApk(req)) {
    return res.json({ ok: false, msg: ANDROID_OLD_NATIVE_APK_MESSAGE });
  }
