...
async function initStore() {
  try {
    if (hasGithubStore()) {
      ...
      deviceDailyStats = normalizeDeviceDailyStats(
        await readGithubStore(MAIN_DEVICE_STATS_GITHUB_DATA_PATH, mainStoreConfig)
      );
      cleanupExpiredMainKeys();
      cleanupDeviceDailyStats();
      ...
      deviceDailyStats = normalizeDeviceDailyStats(loadLocalDeviceDailyStats());
      cleanupExpiredMainKeys();
      cleanupDeviceDailyStats();
      saveLocalStore();
      saveLocalExpiredMainKeys(expiredMainKeys);
      saveLocalDeviceDailyStats(deviceDailyStats);
      lastSyncedMainKeys = cloneStore(keys);
...
<body>
        ...
        <div class="adminStack">
          <div id="adminLogin" class="panelBlock loginCard">
            ...
            <div class="formRow" style="margin-top:12px">
              <button class="btn" style="margin-top:0" onclick="dangNhapAdmin()">Vào dashboard</button>
              <button class="miniBtn" onclick="xoaPhienAdmin()">Xóa phiên</button>
            </div>
            ...
            <div class="panelBlock">
              <div class="toolbarLine">
                ...
                <div class="adminActions">
                  <button class="miniBtn" onclick="taiTongQuan()">Làm mới thống kê</button>
                  <button class="miniBtn" onclick="taiDanhSach()">Làm mới danh sách</button>
                  <button class="miniBtn" onclick="xoaPhienAdmin()">Đăng xuất admin</button>
                </div>
              </div>
            ...
            <div class="adminGrid">
              <div class="panelBlock">
                ...
                <div class="formRow" style="margin-top:12px">
                  ...
                  <button class="miniBtn" onclick="taiDanhSach()">Tải danh sách key</button>
                </div>
              ...
              <div class="panelBlock">
                ...
                <div class="searchRow">
                  ...
                  <button class="miniBtn" onclick="timKey()">Tìm key</button>
                </div>
  ...
  <script>
    ...
    function renderOverview(data) {
      var today = data && data.today ? data.today : { day: "-", admin: { android: 0, ios: 0 }, free: { android: 0, ios: 0 } };
      document.getElementById("activeDay").textContent = "Hôm nay " + today.day;
      var counts = data && data.counts ? data.counts : { adminKeys: 0, freeKeys: 0, expiredDeletedAdminKeys: 0 };
      var stats = [
        { title: "Admin Android hôm nay", value: today.admin.android, meta: "Thiết bị Android dùng key admin" },
        { title: "Admin iOS hôm nay", value: today.admin.ios, meta: "Thiết bị iOS dùng key admin" },
        { title: "Free Android hôm nay", value: today.free.android, meta: "Thiết bị Android lấy key free" },
        { title: "Free iOS hôm nay", value: today.free.ios, meta: "Thiết bị iOS lấy key free" },
        ...
        { title: "Tổng thiết bị hôm nay", value: Number(today.admin.android || 0) + Number(today.admin.ios || 0) + Number(today.free.android || 0) + Number(today.free.ios || 0), meta: "Tổng login hợp lệ theo ngày" }
      ];
      var html = "";
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      document.getElementById("adminKeyInput").addEventListener("keydown", function (event) {
        if (event.key === "Enter") dangNhapAdmin();
      });
      document.getElementById("findKeyInput").addEventListener("keydown", function (event) {
...
app.get("/api/admin/overview", async (req, res) => {
  ...
  const today = getTodayDeviceUsageSummary();
...
app.post("/api/check", async (req, res) => {
  ...
  recordDailyDeviceUsage(record.scope === "free" ? "free" : "admin", detectPlatformFromRequest(req), device);
  saveDeviceDailyStats({ background: true }).catch((err) => {
    console.error("Background save failed:", err.message);
  });
