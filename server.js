...
<head>
  ...
  <script>
    ...
    function dangNhap() {
      var key = keyInputEl ? String(keyInputEl.value || "").trim() : "";
      ...
      try {
        ...
        var url = "/api/check-ios?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device) + "&t=" + Date.now();
        ...
        xhr.onreadystatechange = function () {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            finish();
            setMsg("Lỗi server " + xhr.status + ": " + ((data && (data.msg || data.error)) || "không trả kết quả"), "err");
          }
        };
        ...
        xhr.ontimeout = function () {
          ...
          finish();
          setMsg("Server phản hồi quá lâu, thử lại hoặc kiểm tra Render/GitHub.", "err");
        };
        xhr.send(null);
      } catch (e) {
        done = true;
        finish();
        setMsg("Lỗi JS login: " + (e && e.message ? e.message : e), "err");
      }
    }
