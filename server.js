...
<head>
  ...
  <script>
    ...
    function restoreDevicePresetProfile() {
      ...
      const savedName = String(localStorage.getItem(getDevicePresetDraftKey()) || "").trim();
      ...
      if (!savedName && autoDetectedName) {
        localStorage.setItem(getDevicePresetDraftKey(), autoDetectedName);
      }
      const normalized = normalizePresetDeviceName(resolvedName);
      const resetCount = Number(localStorage.getItem(getDevicePresetResetKey(normalized)) || "0") || 0;
    ...
    function requestDevicePresetProfile(resetSeed) {
      ...
      localStorage.setItem(getDevicePresetDraftKey(), deviceName);
      const normalized = normalizePresetDeviceName(deviceName);
      let resetCount = Number(localStorage.getItem(getDevicePresetResetKey(normalized)) || "0") || 0;
      if (resetSeed) {
        resetCount += 1;
        localStorage.setItem(getDevicePresetResetKey(normalized), String(resetCount));
      }
    ...
    if (devicePresetInput) {
      devicePresetInput.addEventListener("input", function () {
        localStorage.setItem(getDevicePresetDraftKey(), devicePresetInput.value || "");
      });
    }
