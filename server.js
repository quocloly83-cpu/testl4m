...
function githubRequest(method, apiPath, body) {
  return new Promise((resolve, reject) => {
    console.log("ðŸ‘‰ CALL GITHUB:", method, apiPath);
    console.log("ðŸ‘‰ TOKEN PREFIX:", GITHUB_TOKEN ? GITHUB_TOKEN.slice(0, 10) : "(empty)");
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          console.log("ðŸ‘‰ RESPONSE:", (data || "").slice(0, 300));
          let parsed = {};
          try {
            parsed = JSON.parse(data || "{}");
          } catch (parseErr) {
            console.log("âŒ GITHUB JSON PARSE ERROR:", parseErr.message);
            parsed = {};
          }
    ...
    req.on("error", (err) => {
      console.log("âŒ REQUEST ERROR:", err.message);
      reject(err);
    });
...
async function readGithubStore(dataPath = GITHUB_DATA_PATH) {
  console.log("=== READ GITHUB STORE ===");
  console.log("REPO:", GITHUB_REPO);
  console.log("BRANCH:", GITHUB_BRANCH);
  console.log("PATH:", dataPath);
  const [owner, repo] = String(GITHUB_REPO || "").split("/");
  if (!owner || !repo) throw new Error("GITHUB_REPO khÃ´ng há»£p lá»‡");

  const apiPath = `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${dataPath
    .split("/")
  ...
  try {
    ...
    console.log("âŒ READ GITHUB STORE ERROR:", err.statusCode || "no-status", err.message);
...
async function writeGithubStore(
  ...
  const apiPath = `/repos/${encodeURIComponent(
    GITHUB_REPO.split("/")[0]
  )}/${encodeURIComponent(GITHUB_REPO.split("/")[1])}/contents/${dataPath
    .split("/")
    .map(encodeURIComponent)
    .join("/")}`;
...
app.use((req, res, next) => {
  if (!storeReady && !req.path.startsWith("/healthz")) {
    return res.status(503).json({ ok: false, msg: "Store Ä‘ang khá»Ÿi Ä‘á»™ng" });
  }
  next();
});
...
app.use((req, res, next) => {
  ...
  if (arr.length > activeRule.limit) {
    return res.status(429).json({ ok: false, msg: "Thao tÃ¡c quÃ¡ nhanh, chá» xÃ­u rá»“i thá»­ láº¡i" });
  }
...
function formatVNTime(ms) {
  if (isPermanentExpire(ms)) return "VÄ©nh viá»…n";
...
function renderLogo(size, radius) {
  ...
  return `<div style="width:${size}px;height:${size}px;border-radius:${r}px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#8c52ff,#ff70c7);font-size:${Math.round(size * 0.4)}px;color:#fff">âš¡</div>`;
}
...
function baseStyles() {
  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .loginHint{margin:0 auto;color:#ddd2b2;font-size:clamp(14px,2.05vw,16px);line-height:1.78;max-width:34rem}
      ...
      <div class="homeGlass">
        <div class="heroRow">
          <div class="heroIntro">
            <div class="brand">
              ...
              <div>
                <div class="pill">âœ¦ VIP ACTIVE</div>
                <h1 class="heroTitle">AimTrickHead<br>Secure Panel</h1>
                <div class="credit">CRE HUY FANTA Â· AIMTRICKHEAD UPDATE</div>
              </div>
            </div>
        ...
        <div class="loginPanel">
          <div class="loginIntro">
            <h2 class="loginTitle">Truy cáº­p nhanh</h2>
...
<body>
  ...
  <div class="loadingLayer" id="loadingLayer">
    ...
    <div class="loadingSub">Äang Táº£i Chá» XÃ­u...</div>
  ...
  <div class="wrap">
    <div class="card">
      <div class="top">
        <div class="brand">
          ...
          <div>
            ...
            <div class="credit">CRE HUY FANTA Â· AimTrickHead</div>
          </div>
        </div>
      </div>
      ...
      <div class="content">
        ...
        <div class="loginFeatureList"><div class="loginFeatureItem"><span class="loginFeatureTick">âœ“</span><span>Hoáº¡t Äá»™ng á»”n Äá»‹nh MÆ°á»£t MÃ  TrÃªn Adr VÃ  iOS</span></div><div class="loginFeatureItem"><span class="loginFeatureTick">âœ“</span><span>An ToÃ n TÃ i Khoáº£n Game</span></div><div class="loginFeatureItem"><span class="loginFeatureTick">âœ“</span><span>Tá»± Äá»™ng Update</span></div></div>
        <div id="loginView" class="loginHero" style="background:transparent;border:none;border-radius:0;box-shadow:none;min-height:auto;display:flex;align-items:stretch;justify-content:flex-start">
          ...
          <div class="loginIntro">
            ...
            <p class="loginHint">
             APP Há»— Trá»£ KÃ©o TÃ¢m TrÃªn ADR - IOS AimTrickHead.
            </p>
          </div>
          
          <input id="keyInput" class="input" placeholder="Nháº­p Key VÃ o ÄÃ¢y Äi NÃ¨">
          <button class="btn" onclick="dangNhap()">ÄÄƒng nháº­p</button>
        ...
        <div id="panelView" class="hidden">
          <div class="topLine">
            <div class="pill">âœ¦ VIP ACTIVE</div>
            <button class="smallBtn" onclick="dangXuat()">ThoÃ¡t</button>
          </div>

          <div class="quickStats">
            <div class="statChip"><span class="statLabel">Support</span><strong>Há»— Trá»£ FF</strong></div>
            <div class="statChip"><span class="statLabel">Support</span><strong>Há»— Trá»£ FFMAX</strong></div>
            <div class="statChip"><span class="statLabel">Device</span><strong>All Mobile</strong></div>
          </div>

          <div class="noticeBox" id="keyNotice">
            Key Ä‘ang hoáº¡t Ä‘á»™ng.
          </div>
          ...
          <div id="tab1" class="tabPane active">
            <div class="tile"><div class="row"><div><p class="name">AimTrickHead</p><p class="desc">Chá»©c NÄƒng Há»— Trá»£ KÃ©o TÃ¢m Trong APP</p></div><label class="switch"><input type="checkbox" id="f2" onchange="toggleFx(this,'AimTrickHead')"><span class="slider"></span></label></div></div>
            <div class="tile"><div class="row"><div><p class="name">BÃ¡m Äáº§u</p><p class="desc">TÃ¡c dá»¥ng há»— trá»£ bÃ¡m</p></div><label class="switch"><input type="checkbox" id="f3" onchange="toggleFx(this,'BÃ¡m Äáº§u')"><span class="slider"></span></label></div></div>
            <div class="tile"><div class="row"><div><p class="name">Nháº¹ TÃ¢m</p><p class="desc">TÃ¡c dá»¥ng nháº¹ tÃ¢m</p></div><label class="switch"><input type="checkbox" id="f4" onchange="toggleFx(this,'Nháº¹ TÃ¢m')"><span class="slider"></span></label></div></div>
          </div>

          <div id="tab2" class="tabPane">
            <div class="tile"><div class="row"><div><p class="name">Tá»‘i Æ¯u Máº¡nh</p><p class="desc">TÄƒng pháº£n há»“i ngay sau khi báº­t nhen</p></div><label class="switch"><input type="checkbox" id="f5" onchange="toggleFx(this,'Tá»‘i Æ¯u Máº¡nh')"><span class="slider"></span></label></div></div>
            <div class="tile"><div class="row"><div><p class="name">Buff Nháº¡y x Nháº¹ TÃ¢m</p><p class="desc">Tá»‘i Æ°u cáº£m giÃ¡c kÃ©o vÃ  Ä‘á»™ mÆ°á»£t</p></div><label class="switch"><input type="checkbox" id="f6" onchange="toggleFx(this,'Buff Nháº¡y x Nháº¹ TÃ¢m')"><span class="slider"></span></label></div></div>
            <div class="tile"><div class="row"><div><p class="name">Game Boost</p><p class="desc">Tá»‘i Æ¯u MÆ°á»£t Game </p></div><label class="switch"><input type="checkbox" id="f7" onchange="toggleFx(this,'Game Boost')"><span class="slider"></span></label></div></div>
            <div class="tile"><div class="row"><div><p class="name">Nháº¹ TÃ¢m + Fix Rung</p><p class="desc">Giáº£m Rung Nháº¹</p></div><label class="switch"><input type="checkbox" id="f1" onchange="toggleFx(this,'Nháº¹ TÃ¢m + Fix Rung')"><span class="slider"></span></label></div></div>
            <div class="tile">
          ...
          <div id="tab3" class="tabPane">
            <div class="grid2">
              <button class="gameBtn" onclick="prepGame('FF')">ðŸŽ® <span>Free Fire</span></button>
              <button class="gameBtn" onclick="prepGame('FFMAX')">ðŸ”¥ <span>FF MAX</span></button>
            </div>
            <div class="footer">Báº¬T XONG Rá»’I CHIáº¾N NHÃ‰.</div>
          </div>

          <div id="tab4" class="tabPane">
            <div class="grid2">
              <a class="socialBtn" href="${ZALO_URL}" target="_blank" rel="noopener noreferrer">${iconZalo()} <span>LiÃªn há»‡ Zalo</span></a>
              <a class="socialBtn" href="${FACEBOOK_URL}" target="_blank" rel="noopener noreferrer">${iconFacebook()} <span>Facebook</span></a>
            </div>
            <div class="footer">Mua Key VÄ©nh Viá»…n Hoáº·c Há»— Trá»£ LiÃªn Há»‡.</div>
          </div>
          ...
          <div id="tab6" class="tabPane">
            <div class="grid2">
              ...
              <a class="socialBtn" href="${ZALO_URL}" target="_blank" rel="noopener noreferrer">${iconZalo()} <span>LiÃªn há»‡ Admin</span></a>
            </div>
            <div class="footer">
              Theo DÃµi ThÃ´ng Tin Vá» APP , Files Aimlock , KÃ©o TÃ¢m.<br>
              Key VÄ©nh Viá»…n LiÃªn Há»‡.
            </div>
          </div>

          <div class="liveFx" id="liveFxBox"><span class="fxLine">âš¡ Chá» kÃ­ch hoáº¡t module...</span></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="fxOverlay" class="fxOverlay hidden">
    ...
    <div class="fxOverlayCard">
      <div class="fxOverlayTitle" id="fxOverlayTitle">ÄÃƒ Báº¬T</div>
      <div class="fxOverlaySub" id="fxOverlaySub">Chá»©c nÄƒng Ä‘ang hoáº¡t Ä‘á»™ng</div>
    </div>
  </div>
  ...
  <div id="gamePrepOverlay" class="fxOverlay hidden">
    ...
    <div class="fxOverlayCard prepCard">
      ...
      <div class="fxOverlayTitle" id="gamePrepTitle">ÄANG SETUP</div>
      <div class="fxOverlaySub" id="gamePrepSub">Äang cáº¥u hÃ¬nh tÃ­nh nÄƒng...</div>
  ...
  <div id="gameDoneOverlay" class="fxOverlay hidden">
    <div class="fxGlow"></div>
    <div class="fxOverlayCard">
      <div class="fxOverlayTitle">ÄÃƒ SETUP XONG</div>
      <div class="fxOverlaySub" id="gameDoneSub">ThoÃ¡t ra vÃ  vÃ o game lÃ  Ä‘Æ°á»£c nhÃ© bro</div>
    </div>
  </div>
  ...
  <script>
    ...
    function msToViDuration(ms) {
      ...
      if (minutes || parts.length === 0) parts.push(minutes + " phÃºt");
      return parts.slice(0, 3).join(" ");
    }

    function buildNotice(data) {
      const keyText = data.key || getSavedKey() || "Äang hoáº¡t Ä‘á»™ng";
      const remainText = !data.expireAt ? "VÄ©nh viá»…n" : msToViDuration((data.expireAt || 0) - Date.now());
      keyNotice.innerHTML =
        '<b>Key:</b> ' + keyText +
        '<br><b>Hiá»‡u lá»±c cÃ²n:</b> ' + remainText +
        '<br><b>Háº¿t háº¡n lÃºc:</b> ' + (data.expireText || "--");
    }
    ...
    function startFxFeed() {
      ...
      const lines = [
        [{ text: "[" + makeStamp() + "] runtime.idle => premium-ready", tone: "info" }, { text: "â†³ state cache verified", tone: "ok" }, { text: "â†³ running app", tone: "ok" }, { text: "â†³Freefire smooth", tone: "ok" }, { text: "â†³ shell depth = 4.0", tone: "info" }],
        [{ text: "[" + makeStamp() + "] ui.pipeline => stable", tone: "info" }, { text: "â†³  stream active", tone: "ok" }, { text: "â†³ auto update", tone: "ok" }, { text: "â†³ com.dts.ffth", tone: "info" }, { text: "â†³ OK", tone: "ok" }],
        [{ text: "[" + makeStamp() + "] mobile.profile => online", tone: "info" }, { text: "â†³ update all", tone: "ok" }, { text: "â†³ sensivity on", tone: "ok" }, { text: "â†³com.dts.ffmax", tone: "ok" }, { text: "â†³ overscroll bounce = 0", tone: "ok" }],
        [{ text: "[" + makeStamp() + "] optimize.stream => armed", tone: "info" }, { text: "â†³online", tone: "ok" }, { text: "â†³ module  ready", tone: "ok" }, { text: "â†³ cache drift = 0.00", tone: "info" }, { text: "â†³ freefire lock fps", tone: "ok" }],
        [{ text: "[" + makeStamp() + "] premium.tabs => elite", tone: "info" }, { text: "â†³ on files", tone: "ok" }, { text: "â†³ smooth render ", tone: "ok" }, { text: "â†³ shell locked", tone: "warn" }, { text: "â†³ lock fps", tone: "ok" }]
      ];
      let i = 0;
      writeFx(lines[0]);
    ...
    function simulateCodeRun(label, isOn) {
      ...
      const seq = [
        ...
        { text: isOn ? "[" + makeStamp() + "] > status => ACTIVE â€¢ LOCKED â€¢ STABLE" : "[" + makeStamp() + "] > status => OFF â€¢ RELEASED â€¢ SAFE", tone: isOn ? "ok" : "warn" }
      ];
    ...
    function dangXuat() {
      ...
      setMsg("", "");
      showToast("ÄÃ£ thoÃ¡t", "err");
    }

    async function dangNhap() {
      const key = document.getElementById("keyInput").value.trim();
      if (!key) {
        setMsg("Vui lÃ²ng nháº­p key.", "err");
        return;
      }
      setMsg("Äang kiá»ƒm tra key...");
      try {
        ...
        if (!data.ok) {
          setMsg(data.msg || "ÄÄƒng nháº­p tháº¥t báº¡i.", "err");
          return;
        }
        saveSession({ token: data.token, key: key });
        data.key = key;
        setMsg("ÄÄƒng nháº­p thÃ nh cÃ´ng.", "ok");
        showToast("ÄÄƒng nháº­p thÃ nh cÃ´ng", "ok");
        moPanel(data);
      } catch (e) {
        setMsg("KhÃ´ng thá»ƒ káº¿t ná»‘i tá»›i mÃ¡y chá»§.", "err");
      }
    }

    function showBigFx(label, isOn) {
      fxOverlayTitle.textContent = isOn ? "ÄÃƒ Báº¬T" : "ÄÃƒ Táº®T";
      fxOverlaySub.textContent = label + (isOn ? " Ä‘ang hoáº¡t Ä‘á»™ng" : " Ä‘Ã£ ngá»«ng hoáº¡t Ä‘á»™ng");
    ...
    function toggleFx(el, label) {
      ...
      showToast(label + (el.checked ? " Ä‘Ã£ báº­t" : " Ä‘Ã£ táº¯t"), el.checked ? "ok" : "err");
    ...
    function prepGame(target) {
      ...
      gamePrepTitle.textContent = "ÄANG SETUP " + pretty;
      gamePrepSub.textContent = "Äang Ä‘á»“ng bá»™ preset, module vÃ  tráº¡ng thÃ¡i Ä‘Ã£ báº­t";
      gamePrepOverlay.classList.remove("hidden");
      gameDoneOverlay.classList.add("hidden");
      simulateCodeRun("launch_" + pretty, true);
      showToast("Äang setup " + pretty, "ok");

      if (isAndroid()) {
        setTimeout(function () {
          gamePrepOverlay.classList.add("hidden");
          gameDoneSub.textContent = "ÄÃ£ hoÃ n thÃ nh báº­t cÃ¡c chá»‰nh sá»­a data, vui lÃ²ng thoÃ¡t app má»Ÿ game thá»§ cÃ´ng";
          gameDoneOverlay.classList.remove("hidden");
          showToast("ÄÃ£ hoÃ n thÃ nh báº­t cÃ¡c chá»‰nh sá»­a data, vui lÃ²ng thoÃ¡t app má»Ÿ game thá»§ cÃ´ng", "ok");
          setTimeout(function () {
            gameDoneOverlay.classList.add("hidden");
          }, 2800);
        }, 5000);
      ...
      setTimeout(function () {
        gamePrepOverlay.classList.add("hidden");
        gameDoneSub.textContent = pretty + " Ä‘Ã£ setup xong. Äang má»Ÿ game, náº¿u lá»—i thÃ¬ thoÃ¡t app vÃ´ game nhe.";
        gameDoneOverlay.classList.remove("hidden");
        showToast(pretty + " Ä‘Ã£ setup xong", "ok");
        setTimeout(function () {
          launchGame(target);
        }, 550);
        setTimeout(function () {
...
<body>
      ...
      <div class="top">
        <div class="brand">
          ...
          <div>
            ...
            <div class="credit">CRE HUY FANTA Â· AIM TRICK HEAD</div>
          </div>
        </div>
      </div>
      <div class="content">
        <div class="loginHero">
          ...
          <input id="customKey" class="input" style="margin-top:10px" placeholder="Key muá»‘n táº¡o (Ä‘á»ƒ trá»‘ng = tá»± random)">
          <div class="grid2" style="margin-top:10px">
            <input id="uses" class="input" type="number" value="1" placeholder="Sá»‘ thiáº¿t bá»‹ tá»‘i Ä‘a">
            <input id="days" class="input" type="number" value="30" placeholder="Sá»‘ ngÃ y sá»­ dá»¥ng">
          </div>
          <div class="grid2" style="margin-top:10px">
            <button class="btn" style="margin-top:0" onclick="taoKey()">Táº¡o Key</button>
            <button class="smallBtn" style="height:56px" onclick="taiDanhSach()">Táº£i danh sÃ¡ch key</button>
          </div>
  ...
  <script>
    async function taoKey() {
      ...
      result.innerHTML = "Äang táº¡o key...";
      try {
        ...
        const data = await res.json();
        if (!data.ok) {
          result.innerHTML = '<span class="err">â›” ' + (data.error || "Táº¡o key tháº¥t báº¡i") + '</span>';
          return;
        }
        result.innerHTML =
          '<span class="ok">âœ… Táº¡o thÃ nh cÃ´ng</span><br>' +
          'ðŸ”‘ Key: <b>' + data.key + '</b><br>' +
          'ðŸ“± Sá»‘ thiáº¿t bá»‹ tá»‘i Ä‘a: ' + data.totalDevices + '<br>' +
          'â³ Háº¿t háº¡n: ' + data.expireText;
        taiDanhSach();
      } catch (e) {
        result.innerHTML = '<span class="err">âŒ Lá»—i máº¡ng</span>';
      }
    }
    ...
    async function taiDanhSach() {
      ...
      try {
        ...
        const data = await res.json();
        if (!data.ok) {
          box.innerHTML = '<span class="err">â›” ' + (data.error || "KhÃ´ng táº£i Ä‘Æ°á»£c") + '</span>';
          return;
        }
        const entries = data.items || [];
        if (!entries.length) {
          box.innerHTML = "ChÆ°a cÃ³ key nÃ o.";
          return;
        }
        let html = "";
        for (const v of entries) {
          html +=
            ...
            '<div><b>LÆ°á»£t thiáº¿t bá»‹ cÃ²n:</b> ' + v.usesLeft + '</div>' +
            '<div><b>ÄÃ£ dÃ¹ng:</b> ' + v.usedDevices + ' / ' + v.totalDevices + '</div>' +
            '<div><b>Háº¿t háº¡n:</b> ' + v.expireText + '</div>' +
            '<button class="smallBtn" style="margin-top:10px;background:#7a1734;border:none" onclick="xoaKey(\\'' + v.key + '\\')">XÃ³a key</button>' +
            '</div>';
        }
        box.innerHTML = html;
      } catch (e) {
        box.innerHTML = '<span class="err">âŒ Lá»—i máº¡ng</span>';
      }
    }
...
app.get("/api/check", (req, res) => {
  return res.json({ ok: true, msg: "API check hoáº¡t Ä‘á»™ng, hÃ£y dÃ¹ng POST Ä‘á»ƒ kiá»ƒm tra key" });
});
...
app.post("/api/check", async (req, res) => {
  ...
  if (!key || !device) {
    return res.json({ ok: false, msg: "Thiáº¿u key hoáº·c thiáº¿t bá»‹" });
  }
  ...
  return res.json({
    ok: true,
    msg: "ÄÄƒng nháº­p thÃ nh cÃ´ng",
    key,
    token,
...
app.post("/api/status", async (req, res) => {
  ...
  if (!token || !device) {
    return res.json({ ok: false, msg: "Thiáº¿u phiÃªn Ä‘Äƒng nháº­p" });
  }

  const parsed = verifySessionToken(token);
  if (!parsed) {
    return res.json({ ok: false, msg: "PhiÃªn khÃ´ng há»£p lá»‡" });
  }

  if (parsed.device !== device) {
    return res.json({ ok: false, msg: "PhiÃªn khÃ´ng Ä‘Ãºng thiáº¿t bá»‹" });
  }
  ...
  if (!item.devices.includes(device)) {
    return res.json({ ok: false, msg: "Thiáº¿t bá»‹ chÆ°a Ä‘Æ°á»£c cáº¥p quyá»n cho key nÃ y" });
  }
...
app.post("/api/delete", async (req, res) => {
  ...
  const key = String(req.body.key || "").trim();
  if (!keys[key]) {
    return res.json({ ok: false, error: "KhÃ´ng tÃ¬m tháº¥y key" });
  }

  delete keys[key];
  await saveStore();
  return res.json({ ok: true, msg: "ÄÃ£ xÃ³a key" });
});
