...
function baseStyles() {
  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .btn:disabled{opacity:.62;cursor:not-allowed;transform:none!important;filter:grayscale(.18)}
    .btn:disabled{opacity:.62;cursor:not-allowed;transform:none!important;filter:grayscale(.18)}
    .smallBtn{
  ...
  <script>
    ...
    async function dangNhap() {
      ...
      if (!key) {
        ...
        showToast("Vui lòng nhập key", "err", 2200);
        return;
      }
      loginBusy = true;
      if (loginButtonEl) {
      ...
      showToast("Đang kiểm tra key...", "ok", 1800);
      ...
      try {
        ...
        catch (parseErr) { throw new Error("SERVER_TRA_VE_KHONG_PHAI_JSON: " + raw.slice(0, 80)); }
        if (!res.ok || !data.ok) {
          const message = data.msg || data.error || ("Đăng nhập thất bại (HTTP " + res.status + ")");
          setMsg(message, "err");
          showToast(message, "err", 3600);
          return;
        }
        ...
        const message = isAbort ? "Máy chủ phản hồi quá lâu. Kiểm tra Render/GitHub ENV hoặc thử lại sau vài giây." : "Không thể kiểm tra key: " + (e && e.message ? e.message : "lỗi kết nối");
        setMsg(message, "err");
        showToast(message, "err", 4500);
      } finally {
