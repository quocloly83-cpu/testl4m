...
function baseStyles() {
  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .btn:disabled,.smallBtn:disabled,.miniBtn:disabled{opacity:.62;cursor:not-allowed;transform:none!important}
    .smallBtn{
...
<head>
  ...
  <script>
    ...
    // FIX: một số WebView/iOS/Android có thể nuốt sự kiện click/touch khi dùng fast tap.
    // Gắn nhiều lớp fallback để nút Đăng nhập luôn phản hồi.
    if (loginButtonEl) {
      loginButtonEl.onclick = function (event) {
...
<body>
  ...
  <script>
    ...
    function initAdminPage() {
      ...
      adminLoginBtn.onclick = function (event) {
        if (event) event.preventDefault();
        dangNhapAdmin();
        return false;
      };
      adminLoginBtn.addEventListener("pointerup", function (event) {
        if (event) event.preventDefault();
        dangNhapAdmin();
      });
