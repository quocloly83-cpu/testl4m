  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .top .brand::after{
      ...
      bottom:-10px;
      height:1px;
      background:linear-gradient(90deg, rgba(201,176,122,.24), rgba(168,132,255,.14), transparent 72%);
      opacity:.9;
      animation:brandGlowLine 7s ease-in-out infinite;
      pointer-events:none;
    }
    .logoBox{
    ...
    .top .logoBox::after{
      ...
      position:absolute;
      inset:0;
      background:linear-gradient(120deg, transparent 20%, rgba(255,255,255,.20) 50%, transparent 78%);
      transform:translateX(-160%);
      animation:logoSweep 8.8s linear infinite;
      pointer-events:none;
    }
    ...
    .loginFeatureItem{
      ...
      box-shadow:inset 0 1px 0 rgba(255,255,255,.025), 0 12px 26px rgba(0,0,0,.10);
    }
    .loginFeatureItem::after{
      content:"";
      position:absolute;
      inset:0 auto 0 -40%;
      width:32%;
      ...
      animation:featureSweep 9s linear infinite;
      pointer-events:none;
    }
    .loginFeatureItem:nth-child(2)::after{animation-delay:-3s}
    .loginFeatureItem:nth-child(3)::after{animation-delay:-6s}
    .loginFeatureItem:nth-child(2){background:linear-gradient(180deg, rgba(255,255,255,.034), rgba(255,255,255,.013))}
    .loginFeatureItem:nth-child(3){background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.012))}
    }
    .loginFeatureTick{
    ...
    .loginFeatureItem:nth-child(2) .loginFeatureTick{animation-delay:-1.2s}
    .loginFeatureItem:nth-child(3) .loginFeatureTick{animation-delay:-2.4s}
    .loginFrame{
    ...
    .luxBar::after{
      ...
      transform:skewX(-18deg);
      animation:luxSweep 8.2s linear infinite;
      pointer-events:none;
    }
    .pill{
    ...
    @keyframes logoGlow{
      0%,100%{box-shadow:0 18px 44px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.06), 0 0 28px rgba(168,132,255,.12)}
      50%{box-shadow:0 18px 44px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.06), 0 0 44px rgba(214,196,154,.16)}
    }
    @keyframes logoSweep{
      0%{transform:translateX(-160%)}
      100%{transform:translateX(180%)}
    }
    @keyframes brandGlowLine{
      0%,100%{opacity:.35;transform:scaleX(.96);transform-origin:left center}
      50%{opacity:.95;transform:scaleX(1)}
    }
    @keyframes featurePulse{
      0%,100%{box-shadow:0 0 12px rgba(57,225,136,.14);transform:translateY(-1px) scale(1)}
      50%{box-shadow:0 0 20px rgba(57,225,136,.26);transform:translateY(-1px) scale(1.05)}
    }
