...
async function getSupabaseAnalyticsSummary() {
  ...
  const activeRows =
    (await supabaseAnalyticsRequest(
      "GET",
      `ath_active_key_sessions?select=key_value,key_type,device_id&cycle_id=eq.${encodeURIComponent(
        cycle.cycleId
      )}&last_seen=gte.${encodeURIComponent(cutoffIso)}&is_valid=eq.true&or=${encodeURIComponent(
        `(expire_at.is.null,expire_at.gt.${nowIso})`
      )}`
    )) || [];
  ...
  const cycleRows =
    (await supabaseAnalyticsRequest(
      "GET",
      `ath_cycle_device_usage?select=device_id,key_type&cycle_id=eq.${encodeURIComponent(
        cycle.cycleId
      )}`
    )) || [];
  ...
  const activePermanentKeys = new Set();
  activeRows.forEach((row) => {
    const keyValue = String((row && row.key_value) || "").trim();
    if (!keyValue) return;
    activeKeys.add(keyValue);
    if (row.key_type === "permanent") activePermanentKeys.add(keyValue);
    else activeDayKeys.add(keyValue);
  });

  const monthlyUsers = new Set();
  const monthlyDayUsers = new Set();
  const monthlyPermanentUsers = new Set();
  cycleRows.forEach((row) => {
    const deviceId = String((row && row.device_id) || "").trim();
    if (!deviceId) return;
    monthlyUsers.add(deviceId);
    if (row.key_type === "permanent") monthlyPermanentUsers.add(deviceId);
    else monthlyDayUsers.add(deviceId);
  });
  ...
  return {
    ...
    counts: {
      ...
      activeDayKeys: activeDayKeys.size,
      activePermanentKeys: activePermanentKeys.size,
      monthlyUsers: monthlyUsers.size,
      monthlyDayUsers: monthlyDayUsers.size,
      monthlyPermanentUsers: monthlyPermanentUsers.size
    }
  };
}
...
<head>
        ...
        <div id="loginView" class="loginHero">
          <div class="loginFrame">
            ...
            <div class="loginFormStack">
              ...
              <button id="loginButton" class="btn loginPrimaryBtn" type="button" onclick="dangNhap()">Đăng nhập</button>
            </div>
  ...
  <script>
    ...
    function requestJson(method, url, payload, timeoutMs) {
      return new Promise(function (resolve, reject) {
        try {
          ...
          xhr.timeout = Math.max(5000, Number(timeoutMs || 12000));
          ...
          xhr.onreadystatechange = function () {
            ...
            try {
              ...
              return reject(new Error("Server trả dữ liệu không phải JSON: " + String(xhr.responseText || "").slice(0, 80)));
            }
            if (xhr.status >= 200 && xhr.status < 300) return resolve(data);
            reject(new Error((data && (data.msg || data.error)) || ("HTTP " + xhr.status)));
          };
          xhr.onerror = function () { reject(new Error("Không kết nối được server")); };
          xhr.ontimeout = function () { reject(new Error("Server phản hồi quá lâu")); };
    ...
    async function dangNhap() {
      ...
      const oldText = loginButtonEl ? loginButtonEl.textContent : "";
      if (loginButtonEl) {
        loginButtonEl.disabled = true;
        loginButtonEl.textContent = "Đang kiểm tra...";
      }
      ...
      try {
        ...
        const data = isIOS()
          ? await requestJson(
              "GET",
              "/api/check-ios?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device),
              null,
              12000
            )
          : await requestJson("POST", "/api/check", { key: key, device: device }, 12000);

        if (!data || !data.ok) {
          setMsg((data && data.msg) || "Đăng nhập thất bại.", "err");
          showToast((data && data.msg) || "Đăng nhập thất bại", "err", 3500);
          return;
        }
        ...
        const errText = e && e.message ? e.message : "Không thể kết nối tới máy chủ.";
        setMsg(errText, "err");
        showToast(errText, "err", 4200);
      } finally {
        if (loginButtonEl) {
          loginButtonEl.disabled = false;
          loginButtonEl.textContent = oldText || "Đăng nhập";
        }
      }
    }
  ...
  <script>
    ...
    function renderOverview(data) {
      ...
      if (analytics && analytics.enabled && analytics.counts) {
        stats.push(
          { title: "Key đang hoạt động", value: analytics.counts.activeKeys, meta: "Key hợp lệ có last seen trong " + analytics.activeWindowMinutes + " phút" },
          { title: "Key ngày đang hoạt động", value: analytics.counts.activeDayKeys, meta: "Key ngày còn hạn đang online gần đây" },
          { title: "Key vĩnh viễn đang hoạt động", value: analytics.counts.activePermanentKeys, meta: "Key vĩnh viễn đang online gần đây" },
          { title: "User chu kỳ 30 ngày", value: analytics.counts.monthlyUsers, meta: "Máy unique Android + iOS trong chu kỳ " + (analytics.cycle ? analytics.cycle.label : "-") },
          { title: "User key ngày chu kỳ", value: analytics.counts.monthlyDayUsers, meta: "Máy unique đã dùng key ngày trong chu kỳ" },
          { title: "User key vĩnh viễn chu kỳ", value: analytics.counts.monthlyPermanentUsers, meta: "Máy unique đã dùng key vĩnh viễn trong chu kỳ" }
        );
      } else {
        stats.push({
          title: "Dashboard key hoạt động",
          value: "OFF",
          meta: "Supabase analytics chưa bật hoặc đang lỗi, hệ key vẫn chạy bình thường"
        });
      }
      var html = "";
...
app.get("/api/check-ios", async (req, res) => {
  syncExpiredMainKeysInBackground();
  ...
  if (!record) {
    if (isExpiredDeletedMainKey(key)) {
      return res.json({ ok: false, msg: "Key này đã hết hạn, hết lượt nhập, vui lòng xem video mới nhất" });
    }
  ...
  if (item.expireAt && Date.now() >= item.expireAt) {
    if (record.scope === "free" && freeRuntime && typeof freeRuntime.deleteKey === "function") {
      freeRuntime.deleteKey(key, { background: true }).catch((err) => console.error("Background free key delete failed:", err.message));
    } else {
      delete keys[key];
      expiredMainKeys[key] = item.expireAt;
      saveStore({ background: true }).catch((err) => console.error("Background main key delete failed:", err.message));
      saveExpiredMainKeys({ background: true }).catch(() => {});
    }
  ...
  let changed = false;
  if (!alreadyUsed) {
    if (item.usesLeft <= 0) return res.json({ ok: false, msg: "Key Này Đã Hết Lượt Nhập !" });
    item.devices.push(device);
    item.usesLeft -= 1;
    changed = true;
  }

  recordDailyDeviceUsage(record.scope === "free" ? "free" : "admin", "ios", device);
  saveDeviceDailyStats({ background: true }).catch(() => {});

  if (changed) {
    saveKeyRecord(record.scope, key, item, { background: true }).catch((err) => console.error("Background save failed:", err.message));
  }
