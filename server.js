...
<head>
  ...
  <script>
        ...
        fetch(url, options || {})
          .then(function (res) {
            return res.text().then(function (text) {
              ...
              catch (e) { data = { ok: false, msg: "Server trả dữ liệu lỗi: " + text.slice(0, 120) }; }
    ...
    async function dangNhap() {
      ...
      try {
        const device = getDevice();
        const url = "/api/check-ios?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device) + "&t=" + Date.now();
        const result = await requestJson(url, { method: "GET", cache: "no-store" }, 15000);
        const data = result.data || {};
        if (!data.ok) {
          setMsg(data.msg || data.error || ("Đăng nhập thất bại. Mã lỗi: " + result.status), "err");
          return;
        }
        ...
        setMsg("Không nhận được phản hồi từ server. Thử reload app rồi nhập lại key.", "err");
      } finally {
...
async function handleKeyLoginRequest(req, res, input) {
  if (shouldShowAndroidUpdate(req)) {
  ...
  const device = String((input && input.device) || "").trim();
...
app.get("/api/check-ios", async (req, res) => {
  return handleKeyLoginRequest(req, res, req.query || {});
});
