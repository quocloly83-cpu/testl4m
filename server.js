...
<head>
  ...
  <script>
    ...
    function setLoginButtonBusy(isBusy) {
      ...
      loginButtonEl.style.opacity = isBusy ? "0.72" : "";
    ...
    function requestJson(method, url, payload, timeoutMs) {
      return new Promise(function (resolve, reject) {
        ...
        }, Math.max(6000, Number(timeoutMs || 12000)));
        ...
        xhr.timeout = Math.max(6000, Number(timeoutMs || 12000));
    ...
    async function checkKeyWithFallback(key, device) {
      const qs = "?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device) + "&t=" + Date.now();
      const isIosClient = isIOS();

      if (isIosClient) {
        try {
          return await requestJson("GET", "/api/check-ios" + qs, null, 14000);
        } catch (e1) {
          return await requestJson("POST", "/api/check", { key: key, device: device }, 14000);
        }
      }
    ...
    async function dangNhap() {
      ...
      try {
        const device = getDevice();
        const data = await checkKeyWithFallback(key, device);

        if (!data || !data.ok) {
          setMsg((data && (data.msg || data.error)) || "Đăng nhập thất bại.", "err");
          return;
        }
        ...
        setMsg("Không nhận được phản hồi từ máy chủ. Thử lại hoặc kiểm tra Render/GitHub ENV.", "err");
      } finally {
...
async function handleKeyCheck(req, res, payload, options = {}) {
  const skipAndroidUpdate = !!options.skipAndroidUpdate;
  if (!skipAndroidUpdate && shouldShowAndroidUpdate(req)) {
    return res.json({ ok: false, msg: ANDROID_UPDATE_MESSAGE });
  }
  if (!skipAndroidUpdate && shouldBlockOldNativeApk(req)) {
  ...
  const device = String((payload && payload.device) || "").trim();
...
app.get("/api/check-ios", async (req, res) => {
  return handleKeyCheck(req, res, req.query || {}, { skipAndroidUpdate: true });
});

app.post("/api/check", async (req, res) => {
  return handleKeyCheck(req, res, req.body || {}, { skipAndroidUpdate: false });
});
