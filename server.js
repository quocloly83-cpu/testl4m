...
<head>
        ...
        <div id="loginView" class="loginHero">
          <div class="loginFrame">
            ...
            <div class="loginFormStack">
              ...
              <button id="loginButton" class="btn loginPrimaryBtn" type="button">Đăng nhập</button>
            </div>
  ...
  <script>
    ...
    function requestLoginCheck(key, device) {
      return new Promise(function (resolve, reject) {
        const xhr = new XMLHttpRequest();
        const useGet = isIOS();
        const url = useGet
          ? "/api/check-ios?key=" + encodeURIComponent(key) + "&device=" + encodeURIComponent(device) + "&t=" + Date.now()
          : "/api/check";
        xhr.open(useGet ? "GET" : "POST", url, true);
        xhr.timeout = 15000;
        xhr.setRequestHeader("Accept", "application/json");
        if (!useGet) xhr.setRequestHeader("Content-Type", "application/json");
        xhr.onreadystatechange = function () {
          if (xhr.readyState !== 4) return;
          let data = null;
          try { data = JSON.parse(xhr.responseText || "{}"); }
          catch (e) { data = { ok: false, msg: "Server trả dữ liệu lỗi: " + String(xhr.responseText || "").slice(0, 80) }; }
          if (xhr.status >= 200 && xhr.status < 300) return resolve(data);
          resolve(data && data.msg ? data : { ok: false, msg: "Server lỗi " + xhr.status });
        };
        xhr.onerror = function () { reject(new Error("network")); };
        xhr.ontimeout = function () { reject(new Error("timeout")); };
        xhr.send(useGet ? null : JSON.stringify({ key: key, device: device }));
      });
    }
    ...
    async function dangNhap() {
      ...
      loginBusy = true;
      const oldButtonText = loginButtonEl ? loginButtonEl.textContent : "";
      if (loginButtonEl) {
      ...
      try {
        const data = await requestLoginCheck(key, getDevice());
        if (!data || !data.ok) {
        ...
        setMsg(e && e.message === "timeout" ? "Máy chủ phản hồi quá lâu, thử lại." : "Không thể kết nối tới máy chủ.", "err");
      } finally {
        loginBusy = false;
        if (loginButtonEl) {
          loginButtonEl.disabled = false;
          loginButtonEl.textContent = oldButtonText || "Đăng nhập";
        }
      }
    }
